@echo off
echo ========================================
echo RECONSTRUCTION COMPLÈTE BACKEND E-COMPTA-IA
echo ========================================
echo.

echo [1/5] Arrêt de tous les conteneurs...
cd docker
docker-compose down
cd ..
echo.

echo [2/5] Nettoyage des images Docker...
cd docker
docker-compose down --rmi all --volumes --remove-orphans
cd ..
echo.

echo [3/5] Reconstruction de l'image backend...
cd docker
docker-compose build --no-cache backend
cd ..
echo.

echo [4/5] Démarrage des services...
cd docker
docker-compose up -d
cd ..
echo.

echo [5/5] Attente du démarrage (90 secondes)...
timeout /t 90 /nobreak > nul
echo.

echo ========================================
echo TEST DES ENDPOINTS APRÈS RECONSTRUCTION
echo ========================================
echo.

echo Test du Dashboard...
curl -s -f http://localhost:8080/api/dashboard/test
if %errorlevel% neq 0 (
    echo ❌ Dashboard KO
) else (
    echo ✅ Dashboard OK
)
echo.

echo Test des Employés...
curl -s -f http://localhost:8080/api/hr/employees
if %errorlevel% neq 0 (
    echo ❌ Employés KO
) else (
    echo ✅ Employés OK
)
echo.

echo Test des Écritures Comptables...
curl -s -f http://localhost:8080/api/accounting/journal-entries
if %errorlevel% neq 0 (
    echo ❌ Écritures Comptables KO
) else (
    echo ✅ Écritures Comptables OK
)
echo.

echo Test des Clients...
curl -s -f http://localhost:8080/api/third-parties/customers
if %errorlevel% neq 0 (
    echo ❌ Clients KO
) else (
    echo ✅ Clients OK
)
echo.

echo Test des Fournisseurs...
curl -s -f http://localhost:8080/api/third-parties/suppliers
if %errorlevel% neq 0 (
    echo ❌ Fournisseurs KO
) else (
    echo ✅ Fournisseurs OK
)
echo.

echo Test des Congés...
curl -s -f http://localhost:8080/api/hr/leaves
if %errorlevel% neq 0 (
    echo ❌ Congés KO
) else (
    echo ✅ Congés OK
)
echo.

echo ========================================
echo RÉSUMÉ DE LA RECONSTRUCTION
echo ========================================
echo.
echo Reconstruction terminée !
echo.
pause
