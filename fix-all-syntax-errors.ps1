...
Write-Host "=== CORRECTION DE TOUTES LES ERREURS DE SYNTAXE ===" -ForegroundColor Green

# Fonction pour corriger les erreurs de syntaxe
function Fix-AllSyntaxErrors {
    param($filePath)
...
Write-Host "`nCorrection de toutes les erreurs de syntaxe..." -ForegroundColor Yellow

foreach ($file in $javaFiles) {
    $content = Get-Content -Path $file.FullName -Raw -Encoding UTF8 -ErrorAction SilentlyContinue
    if ($content -match '^ackage |^package$|pa`n|ckage com\\.ecomptaia\\.') {
        if (Fix-AllSyntaxErrors $file.FullName) {
            $correctedCount++
        }
    }
}
